...
public class SimpleManipulations {
     ...
     *
     * This method implements this "relabeling," copying pixels from their old
     * coordinates to their new coordinates.
     *
     ...
     * (0, 0) (1, 0) (2, 0) (0, 1) (1, 1) (2, 1)
     *
     * Rotating this bitmap CCW will produce the following bitmap, with relabeled
     * coordinates:
     *
     * (2, 0) (2, 1) (1, 0) (1, 1) (0, 0) (0, 1)
     *
     * Your job is to implement this "relabeling," copying pixels from their old
     * coordinates to their new coordinates.
     *
     * @param pic The original picture to rotate.
     * @return The rotated picture.
    ...
    public static PixelPicture border(PixelPicture pic, int borderWidth, Pixel borderColor) {
        ...
        Pixel[][] src = pic.getBitmap();
        Pixel[][] tgt = new Pixel[h + 2 * borderWidth][w + 2 * borderWidth]; // swap coordinates
        ...
        for (int col = 0; col < w + (2 * borderWidth); col++) {
            for (int row = 0; row < h + (2 * borderWidth); row++) {
                ...
                } else if (row < borderWidth || row >= h + borderWidth) {
                    tgt[row][col] = borderColor;
                } else {
     ...
     * heavily.
     *
     * Different image manipulation programs use different values. We will use the
     * one Photoshop uses: round(0.299*R + 0.587*G + 0.114*B).
     *
     ...
     *
     * After computing the weighted average, create a new pixel with this average as
     * its component values. For example, the reddish color (180, 100, 80) becomes
     * (122, 122, 122).
     *
     * @param pic the original picture
     ...
     *
     * To do this, simply create a new PixelPicture where each color component of
     * each pixel is the inverse of the original value. To invert a color component,
    ...
    public static PixelPicture grayScaleAverage(PixelPicture pic) {
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                Pixel p = bmp[row][col];
                double r = (double) p.getRed();
                double g = (double) p.getGreen();
                double b = (double) p.getBlue();
                double doubleVal = (r + g + b) / 3.0;
                int newVal = (int) Math.round(doubleVal);
                bmp[row][col] = new Pixel(newVal, newVal, newVal);
            }
        }
        return new PixelPicture(bmp);
    }
     ...
     * color. Note that each component of the resulting pixel must have values in
     * the range {@code 0 <= color <= 255}.
     *
     ...
     */
    public static PixelPicture scaleColors(PixelPicture pic, 
            double rfactor, double gfactor, double bfactor) {
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                ...
                double b = (double) p.getBlue();
                double dR = r * rfactor;
                int newR = (int) Math.round(dR);
                double dG = g * gfactor;
                int newG = (int) Math.round(dG);
                double dB = b * bfactor;
                int newB = (int) Math.round(dB);
                bmp[row][col] = new Pixel(newR, newG, newB);
            }
        }
        return new PixelPicture(bmp);
     ...
     *
     * If alpha is 0.5, the weightedAverage is the same as the average of the two
     * numbers. If alpha is 1.0, then x is returned. Likewise, if it is 0.0, than
     * the result is y.
     *
     * @param alpha weight
     ...
     *
     * The weighted average of two int values is given by the static method above.
     * This transformation applies this formula to each color in the corresponding
     * pixels of the two images.
     *
     * The two images must be exactly the same size for this transformation to
     * apply. If the dimensions of the second image do not match those of the first,
     * then the first image is returned unchanged.
     * 
     * @param alpha weight
        ...
        if (w == w2 && h == h2) {
            for (int col = 0; col < w; col++) {
                for (int row = 0; row < h; row++) {
                    ...
                    int g2 = p2.getGreen();
                    int b2 = p2.getBlue();
                    int newR = weightedAverage(alpha, r, r2);
                    int newG = weightedAverage(alpha, g, g2);
                    int newB = weightedAverage(alpha, b, b2);
                    newBmp[row][col] = new Pixel(newR, newG, newB);
                }
            }
     ...
     *
     * We'll do this by calculating the distance from each pixel to the center of
     * the image, and then multiplying the color values of that pixel by one minus
     * the square of that distance. In other words,
     *
     * 1. Find the col,row coordinates of the center pixels of the image, cx and cy.
     * For example, if the image has width 5 and height 3, then the center pixel is
     * located at (2,1). We'll use doubles to represent this value, in order to
     * account for cases where the picture has an even number of pixels. For
     * example, if the width is 4 and the height 6, then the center is at (1.5,
     * 2.5).
     *
     * 2. For each pixel in the image, find its (proportional) distance d from the
     * center. This is the distance from the center in the col and row directions,
     * divided by the total distance from the center to any corner.
     *
     * d = Math.sqrt (dx * dx + dy * dy) / Math.sqrt (cx * cx + cy * cy)
     *
     * 3. Compute the multiplicative factor f from the distance. This factor should
     * be 1.0 at the center of the picture (i.e. d=0) and then decrease to 0.0 near
     * the edges.
     *
     * f = 1.0 - (d * d)
     *
     * 4. Multiply each component of the pixel by the factor f. Use Math.round to
     * convert the result to an integer value before casting to an int.
     *
     * 5. There is one special case. If the distance from the center to any corner
     * is zero (i.e. if the picture contains a single pixel) this method should just
    ...
    public static PixelPicture vignette(PixelPicture pic) {
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                ...
                bmp[row][col] = new Pixel((int) Math.round(bmp[row][col].getRed() * factor),
